...
from app.modules.auth.service import build_user_profile
...
from app.modules.admin.schemas import AdminUserCreate, AdminUserListResponse, AdminUserRead, AdminUserUpdate
...
def list_admin_users(db: Session, *, page: int = 1, page_size: int = 20) -> AdminUserListResponse:
	"""分页查询用户列表。"""

	items, total = list_users(db, page=page, page_size=page_size)
	return AdminUserListResponse(
		items=[build_user_profile(db, user) for user in items],
		total=total,
		page=page,
		page_size=page_size,
	)
...
def create_admin_user(db: Session, data: AdminUserCreate) -> AdminUserRead:
	...
	user = create_user(db, username=data.username, email=data.email, password_hash=get_password_hash(data.password), status=data.status)
	try:
...
def update_admin_user(db: Session, user_id: int, data: AdminUserUpdate) -> AdminUserRead:
	...
	if "username" in update_data or "email" in update_data:
		_ensure_user_unique(db, user_id=user.id, username=update_data.get("username"), email=update_data.get("email"))

	if "password" in update_data and update_data["password"]:
		update_user_password(db, user, get_password_hash(update_data["password"]))

	update_user_profile(db, user, username=update_data.get("username"), email=update_data.get("email"))
