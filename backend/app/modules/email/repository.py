...
def list_email_configs(db: Session, *, page: int = 1, page_size: int = 20) -> tuple[list[EmailConfig], int]:
	query = select(EmailConfig)
	count_query = select(func.count(EmailConfig.id))
	total = db.scalar(count_query) or 0
	items = db.scalars(query.order_by(EmailConfig.id.desc()).offset((page - 1) * page_size).limit(page_size)).all()
	return items, total
...
def get_email_config_by_name(db: Session, name: str) -> EmailConfig | None:
	return db.scalars(select(EmailConfig).where(EmailConfig.name == name)).first()
...
def list_email_templates(db: Session, *, page: int = 1, page_size: int = 20) -> tuple[list[EmailTemplate], int]:
	query = select(EmailTemplate)
	count_query = select(func.count(EmailTemplate.id))
	total = db.scalar(count_query) or 0
	items = db.scalars(query.order_by(EmailTemplate.id.asc()).offset((page - 1) * page_size).limit(page_size)).all()
	return items, total
