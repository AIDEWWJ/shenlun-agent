...
def list_email_config_read_models(db: Session) -> list[EmailConfigRead]:
	return [EmailConfigRead.model_validate(item) for item in list_email_configs(db)]
...
def update_email_config_item(db: Session, config_id: int, data: EmailConfigUpdate) -> EmailConfigRead:
	...
	update_email_config(db, config, **data.model_dump(exclude_unset=True))
	db.commit()
	db.refresh(config)
	return EmailConfigRead.model_validate(config)
...
def list_email_template_read_models(db: Session) -> list[EmailTemplateRead]:
	return [EmailTemplateRead.model_validate(item) for item in list_email_templates(db)]
