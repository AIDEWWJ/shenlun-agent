...
- ORM 模型定义：影响 `Base.metadata.create_all()` 创建的新库
- [backend/sql/schema.sql](../../backend/sql/schema.sql)：影响全量初始化
- [backend/sql/index_optimization.sql](../../backend/sql/index_optimization.sql)：用于给已存在数据库补索引和约束
